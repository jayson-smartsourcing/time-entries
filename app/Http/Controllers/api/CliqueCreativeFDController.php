...
use App\CCFDGroup as CCFDGroup;
use App\CCFDCompany as CCFDCompany;
use App\CCFDAgent as CCFDAgent;
use App\CCFDContact as CCFDContact;
use App\CCFDTicket as CCFDTicket;
use App\CCFDTimeEntryV3 as CCFDTimeEntryV3;


class CliqueCreativeFDController extends Controller
{
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        CCFDGroup $cc_fd_group,
        CCFDCompany $cc_fd_company,
        CCFDAgent $cc_fd_agent,
        CCFDContact $cc_fd_contact,
        CCFDTicket $cc_fd_ticket,
        CCFDTimeEntryV3 $cc_fd_time_entry
    )
    {
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->cc_fd_group = $cc_fd_group;
        $this->cc_fd_company = $cc_fd_company;
        $this->cc_fd_agent = $cc_fd_agent;
        $this->cc_fd_contact = $cc_fd_contact;
        $this->cc_fd_ticket = $cc_fd_ticket;
        $this->cc_fd_time_entry = $cc_fd_time_entry;
    }

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.cc_fd');
        ...
        $this->cc_fd_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->cc_fd_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->cc_fd_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->cc_fd_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->cc_fd_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->cc_fd_agent->bulkInsert($final_data);
            } 

        }
        $this->cc_fd_contact->addAgentsToContacts("cc_fd");
    ...
    public function getAllContacts(){
        ...
        $x = 1;
        $y = 3;
        $this->cc_fd_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->cc_fd_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTicketsV2(){
        ...
        $this->cc_fd_ticket->truncateTable();
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        $this->cc_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                    } 
        ...
        $this->cc_fd_ticket->updateAllFdTickets("cc_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->cc_fd_ticket->bulkDeleteByTicketExportId($ids);
                        $this->cc_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    } 
        ...
        $this->cc_fd_ticket->updateLatestFdTickets("cc_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function updateAll() {
        ...
        $this->cc_fd_contact->addAgentsToContacts("cc_fd");
        $this->cc_fd_agent->deleteDuplicates("cc_fd_agents");
        $this->cc_fd_contact->deleteDuplicates("cc_fd_contacts");
        $this->cc_fd_company->deleteDuplicates("cc_fd_companies");
        $this->cc_fd_group->deleteDuplicates("cc_fd_groups");
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->cc_fd_time_entry->bulkDeleteById($ids);
                        $this->cc_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->cc_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->cc_fd_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
