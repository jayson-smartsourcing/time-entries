...
use App\LHFDTimeEntryV3 as LHFDTimeEntryV3;
...
class LHFDController extends Controller
{
    public function __construct(
        ...
        LHFDTimeEntryV3 $lh_fd_time_entries
    )    
    {  
        $this->guzzle = $guzzle;
    ...
    public function updateAll() {
        ...
        $this->lh_fd_agent->deleteDuplicates("lh_fd_agents");
        $this->lh_fd_contact->deleteDuplicates("lh_fd_contacts");
        $this->lh_fd_company->deleteDuplicates("lh_fd_companies");
        $this->lh_fd_group->deleteDuplicates("lh_fd_groups");
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->lh_fd_time_entries->bulkDeleteById($ids);
                        $this->lh_fd_time_entries->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->lh_fd_time_entries->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->lh_fd_time_entries->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
