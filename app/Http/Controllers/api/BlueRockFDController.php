...
use App\BlueRockFDTimeEntryV3 as BlueRockFDTimeEntryV3;
...
class BlueRockFDController extends Controller
{
    public function __construct(
        ...
        BlueRockFDTimeEntryV3 $blue_rock_time_entry
    )
    {
        $this->guzzle = $guzzle;
        ...
        $this->blue_rock_time_entry = $blue_rock_time_entry;
    }
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->blue_rock_time_entry->bulkDeleteById($ids);
                        $this->blue_rock_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //echo($date_on_or_before);
        //die;
        $this->blue_rock_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
