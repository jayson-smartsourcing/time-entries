...
use App\EstoreFDGroup as EstoreFDGroup;
use App\EstoreFDCompany as EstoreFDCompany;
use App\EstoreFDAgent as EstoreFDAgent;
use App\EstoreFDContact as EstoreFDContact;
use App\EstoreFDTicket as EstoreFDTicket;
use App\EstoreFDTimeEntryV3 as EstoreFDTimeEntryV3;


class EstoreFDController extends Controller
{
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        EstoreFDGroup $estore_group,
        EstoreFDCompany $estore_company,
        EstoreFDAgent $estore_agent,
        EstoreFDContact $estore_contact,
        EstoreFDTicket $estore_ticket,
        EstoreFDTimeEntryV3 $estore_time_entry
    )
    {
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->estore_group = $estore_group;
        $this->estore_company = $estore_company;
        $this->estore_agent = $estore_agent;
        $this->estore_contact = $estore_contact;
        $this->estore_ticket = $estore_ticket;
        $this->estore_time_entry = $estore_time_entry;
    }

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.estore_fd');
        ...
        $this->estore_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->estore_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->estore_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->estore_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->estore_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->estore_agent->bulkInsert($final_data);
            } 
    ...
    public function getAllContacts(){
        ...
        $x = 1;
        $y = 3;
        $this->estore_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->estore_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTicketsV2(){
        ...
        $this->estore_ticket->truncateTable();
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        $this->estore_ticket->bulkInsert($final_data);
                        $final_data = [];
                    } 
        ...
        $this->estore_ticket->updateAllFdTickets("estore_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->estore_ticket->bulkDeleteByTicketExportId($ids);
                        $this->estore_ticket->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    } 
        ...
        $this->estore_ticket->updateLatestFdTickets("estore_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->estore_time_entry->bulkDeleteById($ids);
                        $this->estore_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->estore_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->estore_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
}
