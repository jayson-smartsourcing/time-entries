...
use App\EGFDTimeEntryV3 as EGFDTimeEntryV3;
...
class EGFDController extends Controller
{
    public function __construct(
        ...
        EGFDTimeEntryV3 $eg_fd_time_entry
    )
    {  
        $this->guzzle = $guzzle;
        ...
        $this->eg_fd_time_entry = $eg_fd_time_entry;
    }
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->eg_fd_time_entry->bulkDeleteById($ids);
                        $this->eg_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->eg_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
