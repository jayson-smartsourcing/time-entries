...
use App\JCSFDGroup as JCSFDGroup;
use App\JCSFDCompany as JCSFDCompany;
use App\JCSFDAgent as JCSFDAgent;
use App\JCSFDContact as JCSFDContact;
use App\JCSFDTicket as JCSFDTicket;
use App\JCSFDTimeEntryV3 as JCSFDTimeEntryV3;


class JCSFDController extends Controller
{
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        JCSFDGroup $jcs_fd_group,
        JCSFDCompany $jcs_fd_company,
        JCSFDAgent $jcs_fd_agent,
        JCSFDContact $jcs_fd_contact,
        JCSFDTicket $jcs_fd_ticket,
        JCSFDTimeEntryV3 $jcs_fd_time_entry
    )
    {
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->jcs_fd_group = $jcs_fd_group;
        $this->jcs_fd_company = $jcs_fd_company;
        $this->jcs_fd_agent = $jcs_fd_agent;
        $this->jcs_fd_contact = $jcs_fd_contact;
        $this->jcs_fd_ticket = $jcs_fd_ticket;
        $this->jcs_fd_time_entry = $jcs_fd_time_entry;
    }

    public function getAllGroups() {
       
        $client = new $this->guzzle();
        $data = config('constants.jcs_fd');
        ...
        $this->jcs_fd_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->jcs_fd_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->jcs_fd_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->jcs_fd_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->jcs_fd_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->jcs_fd_agent->bulkInsert($final_data);
            } 
    ...
    public function getAllContacts(){
        ...
        $x = 1;
        $y = 3;
        $this->jcs_fd_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->jcs_fd_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTickets(){
        ...
        $this->jcs_fd_ticket->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    $now = Carbon::now();
                    $group = $this->jcs_fd_group->getDataById($value->group_id);
                ...
                $this->jcs_fd_ticket->bulkInsert($final_data);
    ...
    public function getLatestTicketExport() {
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->jcs_fd_ticket->bulkDeleteByTicketExportId($ids);
                $this->jcs_fd_ticket->bulkInsert($final_data);
                if(count($not_found) > 0) {
                    $this->bp_not_found->bulkInsert($not_found);
                }
    ...
    public function getAllTicketsV2(){
        ...
        $this->jcs_fd_ticket->updateAllFdTickets("jcs_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
        ...
        $this->jcs_fd_ticket->updateLatestFdTickets("jcs_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->jcs_fd_time_entry->bulkDeleteById($ids);
                        $this->jcs_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->jcs_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->jcs_fd_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
