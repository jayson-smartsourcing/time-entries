...
use App\LJHookerFDGroup as LJHookerFDGroup;
use App\LJHookerFDCompany as LJHookerFDCompany;
use App\LJHookerFDAgent as LJHookerFDAgent;
use App\LJHookerFDContact as LJHookerFDContact;
use App\LJHookerFDTicket as LJHookerFDTicket;
use App\LJHookerFDTimeEntryV3 as LJHookerFDTimeEntryV3;

class LJHookerFDController extends Controller
{
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        LJHookerFDGroup $lj_hooker_fd_group,
        LJHookerFDCompany $lj_hooker_fd_company,
        LJHookerFDAgent $lj_hooker_fd_agent,
        LJHookerFDContact $lj_hooker_fd_contact,
        LJHookerFDTicket $lj_hooker_fd_ticket,
        LJHookerFDTimeEntryV3 $lj_hooker_fd_time_entry
    )
    {  
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->lj_hooker_fd_group = $lj_hooker_fd_group;
        $this->lj_hooker_fd_company = $lj_hooker_fd_company;
        $this->lj_hooker_fd_agent = $lj_hooker_fd_agent; 
        $this->lj_hooker_fd_contact = $lj_hooker_fd_contact;
        $this->lj_hooker_fd_ticket = $lj_hooker_fd_ticket;
        $this->lj_hooker_fd_time_entry = $lj_hooker_fd_time_entry;
    }

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.ljh_wl_fd');
        ...
        $this->lj_hooker_fd_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->lj_hooker_fd_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->lj_hooker_fd_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->lj_hooker_fd_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->lj_hooker_fd_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->lj_hooker_fd_agent->bulkInsert($final_data);
            } 
    ...
    public function getAllTicketsV2(){
        ...
        $this->lj_hooker_fd_ticket->truncateTable();
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        $this->lj_hooker_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                    } 
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->lj_hooker_fd_ticket->bulkDeleteByTicketExportId($ids);
                        $this->lj_hooker_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    } 
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->lj_hooker_fd_time_entry->bulkDeleteById($ids);
                        $this->lj_hooker_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    } 
        ...
        $this->lj_hooker_fd_time_entry->bulkDeletePreviousMonth($date);   
        $this->lj_hooker_fd_time_entry->bulkUpdateByNewInsert();  
        return response()->json(['success'=> true], 200);
    }
