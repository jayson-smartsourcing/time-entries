...
use App\NicholsCrowderFDGroup as NicholsCrowderFDGroup;
use App\NicholsCrowderFDCompany as NicholsCrowderFDCompany;
use App\NicholsCrowderFDAgent as NicholsCrowderFDAgent;
use App\NicholsCrowderFDContact as NicholsCrowderFDContact;
use App\NicholsCrowderFDTicket as NicholsCrowderFDTicket;
use App\NicholsCrowderFDTimeEntryV3 as NicholsCrowderFDTimeEntryV3;


class NicholsCrowderFDController extends Controller
{
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        NicholsCrowderFDGroup $nichols_crowder_fd_group,
        NicholsCrowderFDCompany $nichols_crowder_fd_company,
        NicholsCrowderFDAgent $nichols_crowder_fd_agent,
        NicholsCrowderFDContact $nichols_crowder_fd_contact,
        NicholsCrowderFDTicket $nichols_crowder_fd_ticket,
        NicholsCrowderFDTimeEntryV3 $nichols_crowder_fd_time_entry
    )
    {
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->nichols_crowder_fd_group = $nichols_crowder_fd_group;
        $this->nichols_crowder_fd_company = $nichols_crowder_fd_company;
        $this->nichols_crowder_fd_agent = $nichols_crowder_fd_agent;
        $this->nichols_crowder_fd_contact = $nichols_crowder_fd_contact;
        $this->nichols_crowder_fd_ticket = $nichols_crowder_fd_ticket;
        $this->nichols_crowder_fd_time_entry = $nichols_crowder_fd_time_entry;
    }

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.nichols_crowder_fd');
        ...
        $this->nichols_crowder_fd_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->nichols_crowder_fd_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->nichols_crowder_fd_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->nichols_crowder_fd_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->nichols_crowder_fd_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->nichols_crowder_fd_agent->bulkInsert($final_data);
            } 

        }
        $this->nichols_crowder_fd_contact->addAgentsToContacts("nichols_crowder_fd_fd");
    ...
    public function getAllContacts(){
        ...
        $x = 1;
        $y = 3;
        $this->nichols_crowder_fd_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->nichols_crowder_fd_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTicketsV2(){
        ...
        $this->nichols_crowder_fd_ticket->truncateTable();
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        $this->nichols_crowder_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                    } 
        ...
        $this->nichols_crowder_fd_ticket->updateAllFdTickets("nichols_crowder_fd_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->nichols_crowder_fd_ticket->bulkDeleteByTicketExportId($ids);
                        $this->nichols_crowder_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    } 
        ...
        $this->nichols_crowder_fd_ticket->updateLatestFdTickets("nichols_crowder_fd_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function updateAll() {
        ...
        $this->nichols_crowder_fd_contact->addAgentsToContacts("nichols_crowder_fd_fd");
        $this->nichols_crowder_fd_agent->deleteDuplicates("nichols_crowder_fd_fd_agents");
        $this->nichols_crowder_fd_contact->deleteDuplicates("nichols_crowder_fd_fd_contacts");
        $this->nichols_crowder_fd_company->deleteDuplicates("nichols_crowder_fd_fd_companies");
        $this->nichols_crowder_fd_group->deleteDuplicates("nichols_crowder_fd_fd_groups");
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->nichols_crowder_fd_time_entry->bulkDeleteById($ids);
                        $this->nichols_crowder_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->nichols_crowder_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->nichols_crowder_fd_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
