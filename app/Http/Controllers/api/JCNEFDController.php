...
use App\JCNEFDGroup as JCNEFDGroup;
use App\JCNEFDCompany as JCNEFDCompany;
use App\JCNEFDAgent as JCNEFDAgent;
use App\JCNEFDContact as JCNEFDContact;
use App\JCNEFDTicket as JCNEFDTicket;
use App\JCNEFDTimeEntryV3 as JCNEFDTimeEntryV3;



class JCNEFDController extends Controller
{   
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        JCNEFDGroup $jcne_fd_group,
        JCNEFDCompany $jcne_fd_company,
        JCNEFDAgent $jcne_fd_agent,
        JCNEFDContact $jcne_fd_contact,
        JCNEFDTicket $jcne_fd_ticket,
        JCNEFDTimeEntryV3 $jcne_fd_time_entry
    )
    {  
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->jcne_fd_group = $jcne_fd_group;
        $this->jcne_fd_company = $jcne_fd_company;
        $this->jcne_fd_agent = $jcne_fd_agent;
        $this->jcne_fd_contact = $jcne_fd_contact;
        $this->jcne_fd_ticket = $jcne_fd_ticket;
        $this->jcne_fd_time_entry = $jcne_fd_time_entry;
    }

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.jcne_fd');
        ...
        $this->jcne_fd_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->jcne_fd_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->jcne_fd_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->jcne_fd_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->jcne_fd_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->jcne_fd_agent->bulkInsert($final_data);
            } 
    ...
    public function getAllContacts(){
        ...
        $x = 1;
        $y = 3;
        $this->jcne_fd_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->jcne_fd_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTickets(){
        ...
        $this->jcne_fd_ticket->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    $now = Carbon::now();
                    $group = $this->jcne_fd_group->getDataById($value->group_id);
                ...
                $this->jcne_fd_ticket->bulkInsert($final_data);
    ...
    public function getLatestTicketExport() {
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->jcne_fd_ticket->bulkDeleteByTicketExportId($ids);
                $this->jcne_fd_ticket->bulkInsert($final_data);
                if(count($not_found) > 0) {
                    $this->bp_not_found->bulkInsert($not_found);
                }
    ...
    public function getAllTicketsV2(){
        ...
        $this->jcne_fd_ticket->updateAllFdTickets("jcne_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
        ...
        $this->jcne_fd_ticket->updateLatestFdTickets("jcne_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function updateAll() {
        ...
        $this->jcne_fd_agent->deleteDuplicates("jcne_fd_agents");
        $this->jcne_fd_contact->deleteDuplicates("jcne_fd_contacts");
        $this->jcne_fd_company->deleteDuplicates("jcne_fd_companies");
        $this->jcne_fd_group->deleteDuplicates("jcne_fd_fd_groups");
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->jcne_fd_time_entry->bulkDeleteById($ids);
                        $this->jcne_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->jcne_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->jcne_fd_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
