...
use App\DixonFDGroup as DixonFDGroup;
use App\DixonFDCompany as DixonFDCompany;
use App\DixonFDAgent as DixonFDAgent;
use App\DixonFDContact as DixonFDContact;
use App\DixonFDTicket as DixonFDTicket;
use App\DixonFDTimeEntryV3 as DixonFDTimeEntryV3;

class DixonFDController extends Controller
{
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        DixonFDGroup $dixon_fd_group,
        DixonFDCompany $dixon_fd_company,
        DixonFDAgent $dixon_fd_agent,
        DixonFDContact $dixon_fd_contact,
        DixonFDTicket $dixon_fd_ticket,
        DixonFDTimeEntryV3 $dixon_fd_time_entry
    )
    {  
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->dixon_fd_group = $dixon_fd_group;
        $this->dixon_fd_company = $dixon_fd_company;
        $this->dixon_fd_agent = $dixon_fd_agent;
        $this->dixon_fd_contact = $dixon_fd_contact;
        $this->dixon_fd_ticket = $dixon_fd_ticket;
        $this->dixon_fd_time_entry = $dixon_fd_time_entry;
    }

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.dixon_fd');
        ...
        $this->dixon_fd_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->dixon_fd_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->dixon_fd_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->dixon_fd_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->dixon_fd_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->dixon_fd_agent->bulkInsert($final_data);
            } 
    ...
    public function getAllContacts(){
        ...
        $x = 1;
        $y = 3;
        $this->dixon_fd_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->dixon_fd_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTickets(){
        ...
        $this->dixon_fd_ticket->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    $now = Carbon::now();
                    $group = $this->dixon_fd_group->getDataById($value->group_id);
                ...
                $this->dixon_fd_ticket->bulkInsert($final_data);
    ...
    public function getLatestTicketExport() {
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->dixon_fd_ticket->bulkDeleteByTicketExportId($ids);
                $this->dixon_fd_ticket->bulkInsert($final_data);
                if(count($not_found) > 0) {
                    $this->bp_not_found->bulkInsert($not_found);
                }
    ...
    public function getAllTicketsV2(){
        ...
        $this->dixon_fd_ticket->updateAllFdTickets("dixon_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
        ...
        $this->dixon_fd_ticket->updateLatestFdTickets("dixon_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->dixon_fd_time_entry->bulkDeleteById($ids);
                        $this->dixon_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->dixon_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->dixon_fd_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
}
