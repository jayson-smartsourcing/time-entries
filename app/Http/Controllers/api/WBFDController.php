...
use App\WBFDGroup as WBFDGroup;
use App\WBFDCompany as WBFDCompany;
use App\WBFDAgent as WBFDAgent;
use App\WBFDContact as WBFDContact;
use App\WBFDTicket as WBFDTicket;
use App\WBFDTimeEntryV3 as WBFDTimeEntryV3;


class WBFDController extends Controller
{
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        WBFDGroup $wb_fd_group,
        WBFDCompany $wb_fd_company,
        WBFDAgent $wb_fd_agent,
        WBFDContact $wb_fd_contact,
        WBFDTicket $wb_fd_ticket,
        WBFDTimeEntryV3 $wb_fd_time_entry
    )
    {
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->wb_fd_group = $wb_fd_group;
        $this->wb_fd_company = $wb_fd_company;
        $this->wb_fd_agent = $wb_fd_agent;
        $this->wb_fd_contact = $wb_fd_contact;
        $this->wb_fd_ticket = $wb_fd_ticket;
        $this->wb_fd_time_entry = $wb_fd_time_entry;
    }

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.wb_fd');
        ...
        $this->wb_fd_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->wb_fd_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->wb_fd_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->wb_fd_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->wb_fd_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->wb_fd_agent->bulkInsert($final_data);
            } 

        }
        $this->wb_fd_contact->addAgentsToContacts("wb_fd");
    ...
    public function getAllContacts(){
        ...
        $x = 1;
        $y = 3;
        $this->wb_fd_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->wb_fd_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTicketsV2(){
        ...
        $this->wb_fd_ticket->truncateTable();
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        $this->wb_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                    } 
        ...
        $this->wb_fd_ticket->updateAllFdTickets("wb_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->wb_fd_ticket->bulkDeleteByTicketExportId($ids);
                        $this->wb_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    } 
        ...
        $this->wb_fd_ticket->updateLatestFdTickets("wb_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function updateAll() {
        ...
        $this->wb_fd_contact->addAgentsToContacts("wb_fd");
        $this->wb_fd_agent->deleteDuplicates("wb_fd_agents");
        $this->wb_fd_contact->deleteDuplicates("wb_fd_contacts");
        $this->wb_fd_company->deleteDuplicates("wb_fd_companies");
        $this->wb_fd_group->deleteDuplicates("wb_fd_groups");
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->wb_fd_time_entry->bulkDeleteById($ids);
                        $this->wb_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->wb_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->wb_fd_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
