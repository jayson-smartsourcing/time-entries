...
use App\RealhubFDGroup as RealhubFDGroup;
use App\RealhubFDCompany as RealhubFDCompany;
use App\RealhubFDAgent as RealhubFDAgent;
use App\RealhubFDContact as RealhubFDContact;
use App\RealhubFDTicket as RealhubFDTicket;
use App\RealhubFDTimeEntryV3 as RealhubFDTimeEntryV3;

class RealhubFDController extends Controller
{
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        RealhubFDGroup $realhub_fd_group,
        RealhubFDCompany $realhub_fd_company,
        RealhubFDAgent $realhub_fd_agent,
        RealhubFDContact $realhub_fd_contact,
        RealhubFDTicket $realhub_fd_ticket,
        RealhubFDTimeEntryV3 $realhub_fd_time_entry 
    )
    {  
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->realhub_fd_group = $realhub_fd_group;
        $this->realhub_fd_company = $realhub_fd_company;
        $this->realhub_fd_agent = $realhub_fd_agent;
        $this->realhub_fd_contact = $realhub_fd_contact;
        $this->realhub_fd_ticket = $realhub_fd_ticket;
        $this->realhub_fd_time_entry = $realhub_fd_time_entry;
    }

    public function getAllGroups() {

        $client = new $this->guzzle();
        $data = config('constants.realhub_fd');
        ...
        $this->realhub_fd_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->realhub_fd_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->realhub_fd_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->realhub_fd_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->realhub_fd_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->realhub_fd_agent->bulkInsert($final_data);
            } 
    ...
    public function getAllContacts(){
        ...
        $this->realhub_fd_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->realhub_fd_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTickets(){
        ...
        $this->realhub_fd_ticket->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    $now = Carbon::now();
                    $group = $this->realhub_fd_group->getDataById($value->group_id);
                ...
                $this->realhub_fd_ticket->bulkInsert($final_data);
    ...
    public function getLatestTicketExport() {
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->realhub_fd_ticket->bulkDeleteByTicketExportId($ids);
                $this->realhub_fd_ticket->bulkInsert($final_data);
                if(count($not_found) > 0) {
                    $this->bp_not_found->bulkInsert($not_found);
                }
    ...
    public function getAllTicketsV2(){
        ...
        $this->realhub_fd_ticket->updateAllFdTickets("realhub_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
        ...
        $this->realhub_fd_ticket->updateLatestFdTickets("realhub_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function updateAll() {
        ...
        $this->realhub_fd_contact->addAgentsToContacts("realhub_fd");
        $this->realhub_fd_agent->deleteDuplicates("realhub_fd_agents");
        $this->realhub_fd_contact->deleteDuplicates("realhub_fd_contacts");
        $this->realhub_fd_company->deleteDuplicates("realhub_fd_companies");
        $this->realhub_fd_group->deleteDuplicates("realhub_fd_groups");
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->realhub_fd_time_entry->bulkDeleteById($ids);
                        $this->realhub_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->realhub_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->realhub_fd_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
