...
use App\EmuRunFDTimeEntryV3 as EmuRunFDTimeEntryV3;
...
class EmurunFDController extends Controller
{
    public function __construct(
        ...
        EmuRunFDTimeEntryV3 $emurun_fd_time_entry
    ...
    {  
        ...
        $this->emurun_fd_time_entry = $emurun_fd_time_entry;
    }  

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.emurun_fd');
       ...
       // $this->emurun_fd_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->emurun_fd_contact->bulkDeleteByContactId($ids);
    ...
    public function getAllTicketsV2(){
        ...
        $data = config('constants.emurun_fd');
        $api_key = $data["api_key"];
        $three_month_ago = new Carbon("2019-06-26");
        //$three_month_ago = new Carbon("Last Day of September 2018");
        $three_month_ago = $three_month_ago->format("Y-m-d");

        $link = $data["link"]. "/api/v2/tickets?updated_since=".$three_month_ago."&order_by=updated_at&order_type=asc&include=stats&per_page=100";
        $ticket_export_data = array();
        $x = 1;
        $y = 3;
       ...
       // $this->emurun_fd_ticket->truncateTable();
        ...
        $this->emurun_fd_ticket->updateAllFdTickets("emurun_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
        ...
        $this->emurun_fd_ticket->updateLatestFdTickets("emurun_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function updateAll() {
        ...
        $this->emurun_fd_agent->deleteDuplicates("emurun_fd_agents");
        $this->emurun_fd_contact->deleteDuplicates("emurun_fd_contacts");
        $this->emurun_fd_company->deleteDuplicates("emurun_fd_companies");
        $this->emurun_fd_group->deleteDuplicates("emurun_fd_groups");
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->emurun_fd_time_entry->bulkDeleteById($ids);
                        $this->emurun_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->emurun_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->emurun_fd_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
