...
use App\UptickFDGroup as UptickFDGroup;
use App\UptickFDCompany as UptickFDCompany;
use App\UptickFDAgent as UptickFDAgent;
use App\UptickFDContact as UptickFDContact;
use App\UptickFDTicket as UptickFDTicket;
use App\UptickFDTimeEntryV3 as UptickFDTimeEntryV3;


class UptickFDController extends Controller
{
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        UptickFDGroup $uptick_fd_group,
        UptickFDCompany $uptick_fd_company,
        UptickFDAgent $uptick_fd_agent,
        UptickFDContact $uptick_fd_contact,
        UptickFDTicket $uptick_fd_ticket,
        UptickFDTimeEntryV3 $uptick_fd_time_entry
    )
    {
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->uptick_fd_group = $uptick_fd_group;
        $this->uptick_fd_company = $uptick_fd_company;
        $this->uptick_fd_agent = $uptick_fd_agent;
        $this->uptick_fd_contact = $uptick_fd_contact;
        $this->uptick_fd_ticket = $uptick_fd_ticket;
        $this->uptick_fd_time_entry = $uptick_fd_time_entry;
    }

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.uptick_fd');
        ...
        $this->uptick_fd_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->uptick_fd_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->uptick_fd_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->uptick_fd_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->uptick_fd_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->uptick_fd_agent->bulkInsert($final_data);
            } 

        }
        $this->uptick_fd_contact->addAgentsToContacts("uptick_fd");
    ...
    public function getAllContacts(){
        ...
        $x = 1;
        $y = 3;
        $this->uptick_fd_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->uptick_fd_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTicketsV2(){
        ...
        $this->uptick_fd_ticket->truncateTable();
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        $this->uptick_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                    } 
        ...
        $this->uptick_fd_ticket->updateAllFdTickets("uptick_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->uptick_fd_ticket->bulkDeleteByTicketExportId($ids);
                        $this->uptick_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    } 
        ...
        $this->uptick_fd_ticket->updateLatestFdTickets("uptick_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function updateAll() {
        ...
        $this->uptick_fd_contact->addAgentsToContacts("uptick_fd");
        $this->uptick_fd_agent->deleteDuplicates("uptick_fd_agents");
        $this->uptick_fd_contact->deleteDuplicates("uptick_fd_contacts");
        $this->uptick_fd_company->deleteDuplicates("uptick_fd_companies");
        $this->uptick_fd_group->deleteDuplicates("uptick_fd_groups");
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->uptick_fd_time_entry->bulkDeleteById($ids);
                        $this->uptick_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->uptick_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->uptick_fd_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
