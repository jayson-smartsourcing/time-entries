...
use App\AvnuFDGroup as AvnuFDGroup;
use App\AvnuFDCompany as AvnuFDCompany;
use App\AvnuFDAgent as AvnuFDAgent;
use App\AvnuFDContact as AvnuFDContact;
use App\AvnuFDTicket as AvnuFDTicket;
use App\AvnuFDTimeEntryV3 as AvnuFDTimeEntryV3;

class AvnuFDController extends Controller
{
    public function __construct(
        Guzzle $guzzle,
        BPNotFoundId  $bp_not_found,
        EmployeeRef $employee_ref,
        AvnuFDGroup $avnu_fd_group,
        AvnuFDCompany $avnu_fd_company,
        AvnuFDAgent $avnu_fd_agent,
        AvnuFDContact $avnu_fd_contact,
        AvnuFDTicket $avnu_fd_ticket,
        AvnuFDTimeEntryV3 $avnu_fd_time_entry
    )
    {  
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->avnu_fd_group = $avnu_fd_group;
        $this->avnu_fd_company = $avnu_fd_company;
        $this->avnu_fd_agent = $avnu_fd_agent; 
        $this->avnu_fd_contact = $avnu_fd_contact;
        $this->avnu_fd_ticket = $avnu_fd_ticket;
        $this->avnu_fd_time_entry = $avnu_fd_time_entry;
    }


    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.ljh_avnu_fd');
        ...
        $this->avnu_fd_group->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->avnu_fd_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        $this->avnu_fd_company->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->avnu_fd_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        $this->avnu_fd_agent->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->avnu_fd_agent->bulkInsert($final_data);
            } 
    ...
    public function getAllContacts(){
        ...
        $x = 1;
        $y = 3;
        $this->avnu_fd_contact->truncateTable();
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->avnu_fd_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTicketsV2(){
        ...
        $this->avnu_fd_ticket->truncateTable();
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        $this->avnu_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                    } 
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->avnu_fd_ticket->bulkDeleteByTicketExportId($ids);
                        $this->avnu_fd_ticket->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    } 
    ...
    public function updateAll() {
        ...
        $this->avnu_fd_agent->deleteDuplicates("ljh_avnu_fd_agents");
        $this->avnu_fd_contact->deleteDuplicates("ljh_avnu_fd_contacts");
        $this->avnu_fd_company->deleteDuplicates("ljh_avnu_fd_companies");
        $this->avnu_fd_group->deleteDuplicates("ljh_avnu_fd_groups");
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->avnu_fd_time_entry->bulkDeleteById($ids);
                        $this->avnu_fd_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->avnu_fd_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->avnu_fd_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
}
