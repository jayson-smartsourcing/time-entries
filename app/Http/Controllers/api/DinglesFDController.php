...
use App\DinglesFDTimeEntryV3 as DinglesFDTimeEntryV3;
...
class DinglesFDController extends Controller
{
    public function __construct(
        ...
        DinglesFDTimeEntryV3 $dingles_time_entry
    ...
    {  
        ...
        $this->dingles_time_entry = $dingles_time_entry;
    }  

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.dingles_fd');
    ...
    public function getAllTicketsV2(){
        ...
        $this->dingles_ticket->updateAllFdTickets("dingles_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->dingles_time_entry->bulkDeleteById($ids);
                        $this->dingles_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->dingles_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->dingles_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
}
