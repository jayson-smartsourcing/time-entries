...
use App\CWFDGroup as CWFDGroup;
use App\CWFDCompany as CWFDCompany;
use App\CWFDAgent as CWFDAgent;
use App\CWFDContact as CWFDContact;
use App\CWFDTicket as CWFDTicket;
use App\CWFDTimeEntryV3 as CWFDTimeEntryV3;

class CWFDController extends Controller
{
    public function __construct(
        ...
        CWFDGroup $cw_group,
        CWFDCompany $cw_company,
        CWFDAgent $cw_agent,
        CWFDContact $cw_contact,
        CWFDTicket $cw_ticket,
        CWFDTimeEntryV3 $cw_time_entry
    )
    {
        $this->guzzle = $guzzle;
        $this->bp_not_found = $bp_not_found;
        $this->employee_ref = $employee_ref;
        $this->cw_group = $cw_group;
        $this->cw_company = $cw_company;
        $this->cw_agent = $cw_agent;
        $this->cw_contact = $cw_contact;
        $this->cw_ticket = $cw_ticket;
        $this->cw_time_entry = $cw_time_entry;
    }

    public function getAllGroups() {
        $client = new $this->guzzle();
        $data = config('constants.cw_fd');
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->cw_group->bulkInsert($final_data);
            } 
    ...
    public function getAllCompanies() {
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->cw_company->bulkInsert($final_data);
            } 
    ...
    public function getAllAgents(){
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->cw_agent->bulkInsert($final_data);
            } 
    ...
    public function getAllContacts(){
        ...
        for( $i = 1; $i<= $x; $i++ ) {
            ...
            if(count($body) != 0) {
                ...
                $this->cw_contact->bulkInsert($final_data);
            } 
    ...
    public function getAllTicketsV2(){
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        $this->cw_ticket->bulkInsert($final_data);
                        $final_data = [];
                    } 
        ...
        $this->cw_ticket->updateAllFdTickets("cw_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTicketExportV2() {
            ...
            if(count($body) != 0) {
                ...
                foreach($ticket_export_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->cw_ticket->bulkDeleteByTicketExportId($ids);
                        $this->cw_ticket->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    } 
        ...
        $this->cw_ticket->updateLatestFdTickets("cw_fd");
        return response()->json(['success'=> true], 200);
    }
    ...
    public function getLatestTimeEntriesV3() {
            ...
            if(count($body) != 0) {
                ...
                foreach($time_entry_data as $key => $value) {
                    ...
                    if( ($len - 1) > $key && count($final_data) == 50) {
                        ...
                        $this->cw_time_entry->bulkDeleteById($ids);
                        $this->cw_time_entry->bulkInsert($final_data);
                        $final_data = [];
                        $ids = [];
                    }
        ...
        //die;
        $this->cw_time_entry->bulkDeletePreviousMonth($date_after,$date_on_or_before);
        $this->cw_time_entry->bulkUpdateByNewInsert();
        return response()->json(['success'=> true], 200);
    }
}
